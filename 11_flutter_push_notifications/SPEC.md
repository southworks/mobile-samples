...
Agent brief for implementing `11_flutter_push_notifications`. Follow this document as the source of truth for scope, layout, architecture, and Firebase setup. Do not expand beyond the acceptance checklist unless asked.
...
- Sample-local Cursor rules under `11_flutter_push_notifications/.cursor/` if needed; root rule `sample-11-ai-rules-flutter-and-dart.mdc` already scopes to `11_flutter_push_notifications/flutter/**/*.dart`.
...
- **Never** commit the service account file. Document placing it under `11_flutter_push_notifications/secrets/` (gitignored) or an absolute local path.
...
4. Store it locally, for example:
   ...
   ```text
   11_flutter_push_notifications/secrets/fcm-service-account.json
   ```
