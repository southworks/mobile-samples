...
enum StartupConfigurationError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingGoogleServiceInfo:
            return "Falta el archivo GoogleService-Info.plist en AuthorsCollection/Resources."
        case .invalidGoogleServiceInfo:
            return "GoogleService-Info.plist existe, pero no tiene un formato valido."
        case .placeholderGoogleServiceInfo:
            return "GoogleService-Info.plist todavia tiene placeholders. Reemplazalo con el archivo real descargado desde Firebase."
        case .placeholderInfoPlist:
...
final class AuthService {
    ...
    func configureFirebaseIfNeeded() throws {
        ...
        let hasInfoPlistPlaceholders = [infoClientID, infoServerClientID]
            .contains { value in
                value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
                value.contains("YOUR_")
            } ||
    ...
    private func mapToUserReadableError(_ error: Error) -> Error {
        ...
        if nsError.domain.localizedCaseInsensitiveContains("google") ||
            nsError.localizedDescription.localizedCaseInsensitiveContains("google") {
            return NSError(
                domain: nsError.domain,
                code: nsError.code,
                userInfo: [NSLocalizedDescriptionKey: "No se pudo iniciar sesion con Google. Revisa la configuracion OAuth del proyecto."]
            )
        }
